...
地图相关的数据与页面**全部由脚本从游戏本体生成**，你不需要手写任何坐标：

| 产物 | 说明 |
| --- | --- |
| `_data/maps.json` | 世界地图 + 各地点（含按钮的百分比坐标） |
| `_data/map_events.json` | 每张地图的事件打点位置（含图块坐标） |
| `map/<id>.md` | 19 个地点页 |
| `assets/maps/*.png` | 压缩后的地图图片 |
| `tools/map-console-data.js` | 本地控制台的数据 |

地点与按钮位置取自世界地图（`Map028.json`）里的**场所移动事件**，
打点取自各地点地图的事件坐标，图片取自 `outputimg/`。

游戏更新或你想改图之后：

```bash
python tools/build-maps.py      # 等价于 npm run maps
```

**首次使用要装 Pillow**（图片压缩用）。不装也能跑，但会直接复制原图，仓库会大很多：
...
```
game-guides/tools/map-console.html
```

- 左侧选地点，中间显示地图，右上角四个模式：**浏览 / 删除 / 新增 / 加浮窗**
  （也可以按键盘 `1` `2` `3` `4` 切换）
- 「浏览」模式下把鼠标移到打点上，**就能直接看到最终站点上的浮窗效果**
- 改完点「下载 map_overrides.yml」，用它覆盖 `_data/map_overrides.yml`
- 再跑一次 `python tools/build-maps.py`，然后 `npm run publish -- -m "调整地图打点"`

控制台用的坐标和游戏地图编辑器一致（**图块坐标**），点哪里就是哪一格，不用自己算百分比。
改动会存在浏览器本地，刷新不丢；「全部还原」一键回到初始状态。

### 4.3 不想开控制台：直接改覆盖文件

`_data/map_overrides.yml` 支持三种操作，且**重新生成不会被冲掉**：

```yaml
"42":                    # 地图 id 就是网址里的数字：/map/42/ → 42
  remove:                # 去掉打点
    - "19,28"
  add:                   # 补上打点
    - "5,5"
...
每个地点页**最下方有一张「打点坐标对照表」**，照着抄即可。
写错不会静默失败 —— 脚本会指出**行号**并中止，其他文件都不会动。

### 4.4 开关浮窗 / 打点

改 `_data/site_options.yml`，**push 即生效，不需要重新生成地图数据**：

```yaml
world_map:
  tooltip: true     # 世界大地图：悬停显示地名
place_map:
  markers: true     # 地点页：是否画事件打点
  tooltip: false    # 地点页：打点是否显示浮窗（默认关；打开只显示坐标，供对照排查）
```

### 4.5 游戏数据（装备 / 技能 / 状态 / 宠物）

同样由本机脚本从游戏本体生成：

```bash
python tools/build-gamedata.py      # 等价于 npm run gamedata
```

产出 `_data/gamedata/*.json`（四个分类 + 类型表）与 `assets/gamedata/`
（图标雪碧图 + 宠物头像），页面在 `/data/` 下的七个子页。

| 命令 | 作用 |
| --- | --- |
| `npm run gamedata` | 全量生成（含图标解密、压缩、宠物头像裁切） |
| `python tools/build-gamedata.py --no-icons` | 只更新数据，不处理图片 |
| `python tools/build-gamedata.py --include-unnamed` | 连数据库里**名字为空**的条目也收进来 |

几点要知道的：

- **空白占位条目不收录**。游戏数据库里有大量预留空位（武器 137 条、防具 254 条、状态 16 条），
  既没名字也没内容。技能里另有 154 条空名条目但**有公式和效果**（内部引用技能），
  默认也排除。每张表都会标出排除了多少条。
- **图标是解密的**。游戏图片资源用 RPGMV 格式加密，脚本用 `System.json` 里的密钥解密后
  量化压缩（4.04MB → 1.05MB），页面用 CSS 雪碧图显示，不需要切图。
- **宠物头像是从行走图里裁的**。宠物是「角色」不是「物品」，没有图标索引，
  所以脚本从 `img/characters/` 的行走图里取「朝下站立」那一帧。
- **游戏数据本身的遗留问题会照实呈现**。例如有装备引用了 `冥王套装 / 古尸套装 / 强魔套装`
