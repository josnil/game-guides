...
{%- comment -%}
  ⚠️ 进化链这一整段必须用 div 标签加 markdown="0" 包起来，让 Kramdown 跳过 Markdown 处理。
  原因：Liquid 的左裁剪标签（百分号前加减号的那种）会**吃掉标签前面的空白，包括换行**，
  于是标题与紧随其后的 section 标签之间那个空行消失了 ——
  两者被并成同一段，标题里混进了 HTML，后面的缩进内容还会被整段当成代码块转义，
  页面上直接显示原始标签（而类名、元素都在，静态检查照样"通过"）。

  ⚠️⚠️ 本注释必须放在「## 进化链」**之前**。
     注释自己的开标签也是左裁剪标签，同样会吃掉标题后的空行。
     把它放在标题和内容之间，等于白加 markdown="0"。

  ⚠️ 本注释里不要写任何带百分号的标签示例（哪怕只是省略号）：
