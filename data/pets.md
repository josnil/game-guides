...
{%- comment -%}
  ⚠️ 这里必须用 div 标签加 markdown="0" 属性把整段包起来，让 Kramdown 跳过 Markdown 处理。
  原因：Liquid 的左裁剪标签（百分号前加减号的那种）会**吃掉标签前面的空白，包括换行**，
  于是「## 进化链」与紧随其后的 section 标签之间那个空行消失了 ——
  两者被并成同一段，标题变成「进化链」，后面还跟着一串被转义成文字的标签，
  后面的缩进 HTML 也会整段被当成代码块，页面上直接显示原始标签。
